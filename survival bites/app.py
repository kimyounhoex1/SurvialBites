...
def filter():
    ...
    if meetingtype == '배달':
        posts = db.posts.find({"meetingtype": meetingtype})
    else:
        posts = db.posts.find({"meetingtype": meetingtype})
...
def mainpage(user_id):
    ...
    return render_template('main.html', show_profile=False, have_bg_img=False, posts=posts)
...
@app.route('/updatepost', methods=['GET'])
def updatepostpage():
    return
